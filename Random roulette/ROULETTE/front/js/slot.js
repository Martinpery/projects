$(document).ready(function(){ // impératif car la page doit être entièrement généré PHP + HTML, tout doit être en place afin que le ciblage des objet soit possible
	...
	function transitionFVJ () { // Animation pour faire apparâitre le bouton "Faites vos jeux"
		if (nbEleves > 1) {
				$("#fvj").css({
                    "margin-top" : "0px",
					"opacity" : "1",
    ...
    $("#checkAll").change(function () { // A chaque changement de l'état de la checkbox du checkAll
    ...
    // |||||||||||||||||||||||||||||||||| LE CODE IMPORTANT |||||||||||||||||||||||||||||||||||
    function startSlider() { // faire défiler les photos en vertical to top / c'est tout le UL qui bouge
	...
	$("#choose").click(function() { 
            ...
            $("#slot ul li img").css({ // l'image devient clicable donc on change le curseur.
                "cursor" : "pointer"
            });
